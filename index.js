...
const submitSignupHandler = async e => {
 ...
 const usersArray = localStorage.getItem('users')||[]
  ...
  formData["usr-id"]=usersArray.length+11
  users.push(formData);
...
document.querySelector('.login-box').addEventListener('submit', async e => {
  ...
  if(users){
    ...
    (user) =>{
     ...
     return (user['email-signup'] === formData['usr-login'] || user['usr-signup'] === formData['usr-login'])
     }
  );
  }
 console.log(user)
  ////////////// fetch valedat
  let localUserData={}
  localUserData.email = formData['usr-login']
...
if(user){
  ...
  newUser.email = user["email-signup"]
  newUser.name = user['usr-signup']
  newUser.src =`../photos/${imgUser.files[0].name}`
newUser.handleuser =`@${user['usr-signup']}`
newUser.userId = usersArray.length+10
newUser.srcBack = "../photos/photosBack/nestBack.png"
}
  ...
  console.log(newUser)
  localStorage.setItem('users', JSON.stringify(users));
  localStorage.setItem("localUserData",JSON.stringify(newUser))
  // spinner box
